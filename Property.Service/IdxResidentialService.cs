...
namespace Property.Service
{
    public interface IIdxResidentialService
    {
        ...
        List<PropertyModel> GetResidentials(string dbname = "", string userid = "");
    ...
    public class IdxResidentialService : BaseRepository, IIdxResidentialService
    {
        ...
        public List<PropertyModel> GetResidentials(string dbname="",string userid="")
        {
            ...
            try
            {
                using (IDbConnection _db = OpenConnection())
                {
                    ...
                    perameters.Add("@SaleLease", '0');
                    perameters.Add("@dbname", dbname);
                    perameters.Add("@userid", userid);
        ...
        public List<PropertyModel> GetIdxResidentials()
        {
            try
            {
                using (IDbConnection _db = OpenConnection())
                {
                    ...
                    perameters.Add("@PropertyType", "0");
                    perameters.Add("@SaleLease", "0");
                    List<PropertyModel> IdxResidentialList = _db.Query<PropertyModel>("GetResidentialProperties", perameters, commandType: CommandType.StoredProcedure).ToList();
                    return IdxResidentialList;
        ...
        public PropertyModel GetSingleProperty(string MLS)
        {
            ...
            //{
                using (IDbConnection _db = OpenConnection())
                {
                    ...
                    perameters.Add("@PropertyType", '0');
                    perameters.Add("@SaleLease", '0');
                    PropertyModel IdxResidential = _db.Query<PropertyModel>("GetResidentialProperties", perameters, commandType: CommandType.StoredProcedure).FirstOrDefault();
                    try
                    {
                        string sourcePath;
        ...
        public List<PropertyType> GetPropertyTypes()
        {
            ...
            try
            {
                using (IDbConnection _db = OpenConnection())
                {
                    PropertyTypes = _db.Query<PropertyType>("GetPropertyTypes", commandType: CommandType.StoredProcedure).ToList();
